...
def readLocalFiles(root,subFolder='',debug=1):
	start = datetime.now()
	if debug: drawTitle('Reading local files')
	...
	while len(folderPool)>0:
		...
		while not listover:
			...
			while not succes and times<10:
				...
				try:
					list_folder_ret = cos_client.list_folder(request) #每次请求有199限制
...
def filterModifiedLocalFiles(localFilesDict,cosFilesDict,debug=1):
	if debug: drawTitle('Filtering modified local files')
...
def filterExtraCosFiles(localFilesDict,cosFilesDict,debug=1):
	if debug: drawTitle('Filtering extra files on COS')
...
def syncEmptyFolders(cos_client,bucket,localEmptyFolders,cosEmptyFolders,debug=1):
	start = datetime.now()
	if debug:
	...
	if createFolderNum: print('Create folder(s): %s'%(createFolderNum))

	for folder in cosEmptyFolders:
		deleteCosFolder(cos_client,bucket,folder)
	if cosEmptyFolders: print('Delete folder(s): %s'%len(cosEmptyFolders))
...
def syncLocalToCOS(appid,secret_id,secret_key,bucket,region_info,root,subFolder='',debug=1):
	...
	if list_folder_ret['message']!='SUCCESS':
		raise Exception('>>> Check your appid / secret_id / secret_key / bucket_name <<<')

	# 读取本地需要更新的目录
	localFilesDict,localEmptyFolders = readLocalFiles(root,subFolder,debug=debug)

	# 读取cos上需要更新的目录
	cosFilesDict,cosEmptyFolders = readCosFiles(cos_client,bucket,subFolder,debug=debug)

	# 比较文件修改时间，筛选出需要更新的文件，并且上传。
	modifiedLocalFiles = filterModifiedLocalFiles(localFilesDict,cosFilesDict,debug=debug)
	if modifiedLocalFiles:
		start = datetime.now()
	...
	extraCosFiles = filterExtraCosFiles(localFilesDict,cosFilesDict,debug=debug)
	if extraCosFiles:
		start = datetime.now()
	...
	# 同步空文件夹
	syncEmptyFolders(cos_client,bucket,localEmptyFolders,cosEmptyFolders,debug=debug)
